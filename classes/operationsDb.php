...
var_dump($pdo);
echo '<br>123';
//все равно в классе приходит ебаный NULL когда даже в методе прописываешь global $pdo;
//Витя, че за хуйня? Видмо я запутался с областью видимости вне класса и внутри класса.
//внешнюю переменную я засунуть в класс же не могу?
//И вот так вот вызвать в классе не могу
//protected $pdo = self::getDbh(); или через parent::getDbh(); тоже ошибка
//в итоге не могу получить объект $pdo внутри методов класса
//сейчас то все работает, но я тащу объект $pdo накостылив статичный метод -
// public static function connect()
//и вызываю его в каждом методе класса - $pdo = self::connect();
//как мне всего этого избежать, чтоб один раз прописать вызов объекта класса работы с БД
//и уже спокойно работаь с PDO?
//класс обертка для PDO создан. В папке classes/db.php
//чет я запарился
